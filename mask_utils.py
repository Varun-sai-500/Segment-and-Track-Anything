...
_palette = [0, 0, 0] + ((rng.random(3 * 255) * 0.7 + 0.3) * 255).astype(np.uint8).tolist()
_PALETTE = np.asarray(_palette, dtype=np.uint8).reshape(-1, 3)
...
def draw_mask(img, mask, alpha=0.5):
    ...
    img[binary_mask] = (pixels * inv_alpha + colors * alpha).astype(img.dtype)
    return img
